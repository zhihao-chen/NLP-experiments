...
class Ngrams(object):
    ...
    def intersection(self, o):
        if self.exclusive:
            inter_set = self.ngrams.intersection(o.ngrams)
            return Ngrams(inter_set, exclusive=True)
        else:
            other_list = deepcopy(o.ngrams)
            inter_list = []

            for e in self.ngrams:
                try:
    ...
    def union(self, *ngrams):
        if self.exclusive:
            union_set = self.ngrams
            for o in ngrams:
                union_set = union_set.union(o.ngrams)
            return Ngrams(union_set, exclusive=True)
        else:
            union_list = deepcopy(self.ngrams)
            for o in ngrams:
                union_list.extend(o.ngrams)
...
class Metric(object):
    ...
    @staticmethod
    def f_r_p_rouge_n(reference_count, overlapping_count):

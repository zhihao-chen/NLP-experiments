...
"""
"""
https://github.com/YunwenTechnology/Unilm/blob/63a8cd25e4fd0a7a42e7976490e245ac1a94e332/utils_seq2seq.py
"""
...
class Seq2SeqDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, file, batch_size, tokenizer, max_len, short_sampling_prob=0.1, sent_reverse_order=False,
                 bi_uni_pipeline=[], source="src_txt", target="tgt_txt"):
        ...
        self.ex_list = []
        # 每一行是一个json字符串：{'src_text': "", 'tgt_text': ""}
        file_data = open(file, "r", encoding='utf-8')

        threads = min(8, cpu_count())
        with Pool(threads) as p:
            annotate_ = partial(self.read_data, source=self.source, target=self.target)
            self.ex_list = list(
                tqdm(
    ...
    @staticmethod
    def read_data(line, source, target):
        """
        {'src_text': "", 'tgt_text': ""}
        :param line:
        :param source:
        ...
        src_tk = sample[source]
        tgt_tk = sample[target]
    ...
    def __iter__(self):  # iterator to load data
        for i in range(math.ceil(len(self.ex_list) / float(self.batch_size))):
            batch = []
...
class Preprocess4Seq2seq(Pipeline):
    ...
    def __init__(self, max_pred, mask_prob, vocab_words, indexer, max_len=512, skipgram_prb=0, skipgram_size=0,
        ...
        super(Preprocess4Seq2seq).__init__()
...
class Preprocess4BiLM(Pipeline):
    ...
    def __init__(self, max_pred, mask_prob, vocab_words, indexer, max_len=512, skipgram_prb=0, skipgram_size=0,
                 mask_whole_word=False, mask_source_words=True, tokenizer=None):
        super(Preprocess4BiLM).__init__()
        self.max_len = max_len
        self.max_pred = max_pred  # max tokens of prediction
        self.mask_prob = mask_prob  # masking probability
        self.vocab_words = vocab_words  # vocabulary (sub)words
...
class Preprocess4RightLM(Pipeline):
    ...
    def __init__(self, max_pred, mask_prob, vocab_words, indexer, max_len=512, skipgram_prb=0,
                 skipgram_size=0, mask_whole_word=False, mask_source_words=True, tokenizer=None):
        super(Preprocess4RightLM).__init__()
        self.max_len = max_len
        self.max_pred = max_pred  # max tokens of prediction
        self.mask_prob = mask_prob  # masking probability
        self.vocab_words = vocab_words  # vocabulary (sub)words

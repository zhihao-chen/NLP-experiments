...
"""
# 用于存储和加载training dynamic
# 参考自：https://github.com/allenai/cartography/blob/main/cartography/selection/selection_utils.py
import json
import logging
import os
import pandas as pd
...
def log_training_dynamics(output_dir: os.path,
  ...
  """
    ...
    epoch_file_name = os.path.join(logging_dir, f"dynamics_epoch_{epoch}.json")
...
def read_training_dynamics(model_dir: os.path,
  ...
  """
    ...
    for epoch_num in tqdm.tqdm(range(num_epochs)):
        ...
        with open(epoch_file, "r", encoding='utf8') as infile:
            for line in infile:
                line = line.strip()
                if not line:
                    continue

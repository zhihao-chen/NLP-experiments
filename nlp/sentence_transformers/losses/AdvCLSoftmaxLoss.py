...
class AdvCLSoftmaxLoss(nn.Module):
    ...
    def __init__(self,
                 ...
                 data_augmentation_strategy: str = "adv",
                 # 数据增强策略，可选项：不进行增强“none”、对抗“adv”、mean和max pooling对比“meanmax”、
                 # 随机打乱token位置信息“shuffle”、随机去除embedding某些维度信息“cutoff”、
                 # shuffle和cutoff策略分别生成两个view“shuffle-cutoff”
                 ...
                 data_augmentation_strategy_final_1: str = None,
                 # 最终的五种数据增强方法（none、shuffle、token-cutoff、feature-cutoff、dropout），用于生成第一个view
                 data_augmentation_strategy_final_2: str = None,
                 # 最终的五种数据增强方法（none、shuffle、token-cutoff、feature-cutoff、dropout），用于生成第二个view
        ...
        assert data_augmentation_strategy in (
        "none", "adv", "meanmax", "shuffle", "cutoff", "shuffle-cutoff", "shuffle+cutoff", "shuffle_embeddings", "span")
        ...
        if mapping_to_small_space is not None:
            ...
            self.projection = MLP(sentence_embedding_dimension, projection_hidden_dim, mapping_to_small_space,
                                  use_bn=projection_use_batch_norm)
        else:
        ...
        if add_contrastive_predictor is not None:
            ...
            self.projection = MLP(sentence_embedding_dimension, projection_hidden_dim, sentence_embedding_dimension,
                                  use_bn=projection_use_batch_norm)
        else:
            self.projection_mode = "none"
    ...
    def forward(self, sentence_features: Iterable[Dict[str, Tensor]], labels: Tensor):
        ...
        if not self.training:  # 验证阶段或预测阶段
            ...
            if self.data_augmentation_strategy_final_1 is None:
                if self.use_adversarial_training or (
                        self.use_contrastive_loss and self.data_augmentation_strategy == "adv"):  # 若需要用到对抗训练，或对比学习需要生产对抗样本做数据增强，就生成对抗样本
                    # 1. normal forward
                    sentence_feature_a, sentence_feature_b = sentence_features

                    ori_feature_keys = set(
                        sentence_feature_a.keys())  # record the keys since the features will be updated
                    ...
                    if self.do_noise_normalization:  # do normalization
                        ...
                        normalized_noise_a = unnormalized_noise_a / (
                                    norm_a.unsqueeze(dim=-1) + 1e-10)  # add 1e-10 to avoid NaN
                        norm_b = unnormalized_noise_b.norm(p=2, dim=-1)
                        normalized_noise_b = unnormalized_noise_b / (
                                    norm_b.unsqueeze(dim=-1) + 1e-10)  # add 1e-10 to avoid NaN
                ...
                rep_a_view2 = self._data_aug(sentence_feature_a, self.data_augmentation_strategy_final_2,
                                             ori_feature_keys, self.cutoff_rate_final_2)
                if not self.no_pair:
                    rep_b_view1 = self._data_aug(sentence_feature_b, self.data_augmentation_strategy_final_1,
                                                 ori_feature_keys, self.cutoff_rate_final_1)
                    rep_b_view2 = self._data_aug(sentence_feature_b, self.data_augmentation_strategy_final_2,
                                                 ori_feature_keys, self.cutoff_rate_final_2)
                else:
                    rep_b_view1 = None
                    rep_b_view2 = None
            ...
            if self.use_adversarial_training:
                ...
                self.model.tensorboard_writer.add_scalar(f"train_adv_loss",
                                                         self.adversarial_loss_rate * adv_rate * adv_training_loss.item(),
                ...
                self.model.tensorboard_writer.add_scalar(f"train_normal_loss", loss_n_n.item() * adv_rate,
                                                         global_step=self.model.global_step)
            ...
            if self.use_contrastive_loss:
                ...
                self.model.tensorboard_writer.add_scalar(f"train_contrastive_loss_total",
                                                         self.contrastive_loss_rate * cl_rate * contrastive_loss.item(),
                                                         global_step=self.model.global_step)
                if self.regularization_term_rate > 1e-10:
                    ...
                    self.model.tensorboard_writer.add_scalar(f"contrastive_loss_regularization_term",
                                                             self.regularization_term_rate * regularization_term.item(),
                                                             global_step=self.model.global_step)

...
class GGNNv2(nn.Module):
    def __init__(self, input_dim, time_step,  in_matrix=None, out_matrix=None):
        super(GGNNv2, self).__init__()
        self.input_dim = input_dim
        self.time_step = time_step
        # self._in_matrix = in_matrix
        # self._out_matrix = out_matrix
...
class LabelFusionLayerForToken(nn.Module):
    def __init__(self, hidden_size, label_num, label_emb_size=300):
        super(LabelFusionLayerForToken, self).__init__()
    ...
    def forward(self, token_embs, label_embs, input_mask, label_input_mask=None,
                return_scores=False, use_attn=False, do_add=False, average_pooling=False):
        if use_attn:
            if average_pooling:
                return self.get_fused_feature_with_average_pooling(token_embs, label_embs, input_mask, label_input_mask)
            else:
                return self.get_fused_feature_with_attn(token_embs, label_embs, input_mask,
                                                        label_input_mask, return_scores=return_scores)
        elif do_add:
            return self.get_fused_feature_with_add(token_embs, label_embs, input_mask)
        else:
            return self.get_fused_feature_with_cos_weight(token_embs, label_embs,
                                                          input_mask, return_scores=return_scores)
    ...
    def get_fused_feature_with_average_pooling(self, token_feature, label_feature):
        """
            token_feature: [batch_size, context_seq_len, hidden_dim]
            label_feature: [class_num, label_seq_len, hidden_dim]
        """
    ...
    def get_fused_feature(self, token_feature, label_feature):
        batch_size, seq_len = token_feature.shape[:2]
        token_features = token_feature.contiguous().view(
        ...
        label_features = self.fc_2(label_feature).unsqueeze(
            1).repeat(1, seq_len, 1, 1).view(batch_size*seq_len, self.label_num, self.hidden_size)
        # [bs * seq_len * label_num, hidden_dim]
        temp_features = self.fc_3(torch.tanh(
            token_features*label_features)).contiguous().view(batch_size, seq_len, -1)
        return temp_features


class LabelFusionLayerForClassification(nn.Module):
    def __init__(self, hidden_size, label_num, label_emb_size=300):
        super(LabelFusionLayerForClassification, self).__init__()
        self.label_num = label_num
        self.hidden_size = hidden_size
        self.fc_1 = nn.Linear(self.hidden_size, self.hidden_size, bias=False)
        self.fc_2 = nn.Linear(label_emb_size, self.hidden_size, bias=False)
    ...
    def forward(self, token_embs, label_embs, input_mask=None):
        # [bs, seq_len, hidden_size]; [bs, label_num, 300]

        return self.get_fused_feature_with_cos(token_embs, label_embs, input_mask)

    @staticmethod
    def get_fused_feature_with_cos(token_feature, label_feature, input_mask=None):
...
class LabelFusionLayerForSeq(nn.Module):
    def __init__(self, hidden_size, label_num, label_emb_size=300):
        super(LabelFusionLayerForSeq, self).__init__()
        self.label_num = label_num
        self.hidden_size = hidden_size
        self.fc_1 = nn.Linear(self.hidden_size, self.hidden_size, bias=False)
        self.fc_2 = nn.Linear(label_emb_size, self.hidden_size, bias=False)
...
class LabelAttnFusionLayerForToken(nn.Module):
    def __init__(self, hidden_size, label_num, num_attention_heads, attention_probs_dropout_prob):
        super(LabelAttnFusionLayerForToken, self).__init__()
        self.label_num = label_num
        self.hidden_size = hidden_size

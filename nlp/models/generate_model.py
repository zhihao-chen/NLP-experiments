...
class AdvContrastiveT5(nn.Module):
    """
    基于对比学习的条件文本生成模型
    参考：https://github.com/seanie12/CLAPS/blob/main/src/qg/models.py
    """
    def __init__(self, model_name_or_path=None, config=None, tau=0.1, pos_eps=0.3, neg_eps=1.0):
        ...
        if model_name_or_path is not None:
            ...
            self.t5_model = T5ForConditionalGeneration(config=config)
        self.config = config
        self.embed_dim = self.config.hidden_size
        self.projection = nn.Sequential(nn.Linear(self.embed_dim, self.embed_dim), nn.ReLU())
    ...
    def forward(self, input_ids, attention_mask, decoder_input_ids, decoder_attention_mask,
        ...
        encoder_input_ids, encoder_attention_mask = input_ids, attention_mask
        lm_labels = labels
        ...
        if adv:
            ...
            avg_doc = self.avg_pool(proj_enc_h, encoder_attention_mask)
            ...
            pos_dec_hidden = self.generate_cont_adv(hidden_states, encoder_attention_mask,
            ...
            identity = torch.eye(batch_size, device=encoder_input_ids.device)
            ...
            labels = torch.arange(batch_size, device=encoder_input_ids.device)
    ...
    def generate_cont_adv(self, enc_hiddens, enc_mask, dec_hiddens, dec_mask, lm_logits, tau, eps):
        ...
        true_probs = nnf.softmax(lm_logits, -1)
        true_probs = true_probs * dec_mask.unsqueeze(-1).float()

        perturb_log_probs = nnf.log_softmax(perturb_logits, -1)

...
class BertSelfAttention(nn.Module):
    ...
    def transpose_for_scores(self, x):
        # [bs, seq_len, num_att_h, att_h_size]
        new_x_shape = x.size()[:-1] + (self.num_attention_heads, self.attention_head_size)
        x = x.view(*new_x_shape)
        return x.permute(0, 2, 1, 3)  # [bs, num_att_h, seq_len, att_h_size]
...
class NeZhaSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.relative_positions_embeddings = _generate_relative_positions_embeddings(
            seq_length=512, embed_dim=self.attention_head_size, max_relative_position=config.max_relative_position)
        self.dropout = nn.Dropout(config.attention_probs_dropout_prob)

    def transpose_for_scores(self, x):
        new_x_shape = x.size()[:-1] + (self.num_attention_heads, self.attention_head_size)
        x = x.view(*new_x_shape)
        return x.permute(0, 2, 1, 3)  # [bs, num_att_h, seq_len, att_hs]
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))  # [bs, num_att_h, seq_len, seq_len]
        ...
        relations_keys = self.relative_positions_embeddings.detach().clone()[:to_seq_length, :to_seq_length, :].to(
            device)  # [att_hs, att_hs, att_hs]
        # relations_keys = embeddings.clone().detach().to(device)
        query_layer_t = query_layer.permute(2, 0, 1, 3)  # [seq_len, bs, num_att_h, att_hs]
        query_layer_r = query_layer_t.contiguous().view(from_seq_length, batch_size * num_attention_heads,
                                                        self.attention_head_size)  # [seq_len, bs*num_att_h, att_hs]
        key_position_scores = torch.matmul(query_layer_r, relations_keys.permute(0, 2, 1))
        key_position_scores_r = key_position_scores.view(from_seq_length, batch_size,
                                                         num_attention_heads, from_seq_length)
        key_position_scores_r_t = key_position_scores_r.permute(1, 2, 0, 3)  # [bs, num_att_h, seq_len, seq_len]
        ...
        context_layer = torch.matmul(attention_probs, value_layer)  # [bs, num_att_h, seq_len, att_hs]

...
from transformers.file_utils import WEIGHTS_NAME, is_sagemaker_dp_enabled, is_apex_available
...
class ChildTuningDTrainer(Trainer):
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        ...
        if self.optimizer is None:
            ...
            self.optimizer = optimizer_cls(optimizer_grouped_parameters, mode=self.mode, **optimizer_kwargs)  # noqa
    ...
    def train(
            ...
            trial: Union["optuna.Trial", Dict[str, Any]] = None,  # noqa
            **kwargs,
    ):
        """
        ...
        train_dataset_is_sized = isinstance(self.train_dataset, collections.abc.Sized)  # noqa
        ...
        if self.args.deepspeed:
            ...
            self.deepspeed = model  # DeepSpeedEngine object  # noqa
            self.optimizer = optimizer
            self.lr_scheduler = lr_scheduler  # noqa
        else:
        ...
        if self.use_apex:
            model, self.optimizer = amp.initialize(model, self.optimizer, opt_level=self.args.fp16_opt_level)  # noqa
        ...
        if self.sharded_ddp:
            ...
            model = DDP(model, device_ids=[dist.get_local_rank()], broadcast_buffers=False)  # noqa
        ...
        if resume_from_checkpoint is not None and os.path.isfile(
            ...
            self.state = TrainerState.load_from_json(os.path.join(resume_from_checkpoint, "trainer_state.json"))  # noqa
        ...
        self._total_loss_scalar = 0.0  # noqa
        self._globalstep_last_logged = self.state.global_step  # noqa
        self._total_flos = self.state.total_flos  # noqa
        model.zero_grad()
        ...
        if self.args.load_best_model_at_end and self.state.best_model_checkpoint is not None:
            ...
            if isinstance(self.model, PreTrainedModel):
                ...
                if not self.is_model_parallel:
                    self.model = self.model.to(self.args.device)  # noqa
            else:
        ...
        self.control = self.callback_handler.on_train_end(self.args, self.state, self.control)  # noqa

...
class AdaFactor(Optimizer):
    ...
    @staticmethod
    def _experimental_reshape(shape):
        temp_shape = shape[2:]
        if len(temp_shape) == 1:
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                g = grad
                if group['enable_momentum']:
                    g = torch.div(exp_avg, 1 - beta1_t ** state['step'])  # noqa
                if group['ams_grad']:
                    torch.max(exp_avg_sq_hat, v, out=exp_avg_sq_hat)  # noqa
                    v = exp_avg_sq_hat
                ...
                p.data.add_(-lr_t * (u.view(old_shape) if is_need_reshape and  # noqa

                    ...
                    for param_group in self.optimizer.param_groups:
                        ...
                        if param_group['weight_decay'] != 0:
                            ...
                            if old_weight_decay > new_weight_decay + self.eps:
                                ...
                                if self.verbose:
                                    print('\nEpoch {epoch}: reducing weight decay factor of '
                                          'group {i} to {new_weight_decay:.4e}.')
                    self.cooldown_counter = self.cooldown
                    self.wait = 0
                self.wait += 1
...
class CosineLRWithRestarts(object):
    ...
    def __init__(self, optimizer, batch_size, epoch_size, restart_period=100,
            ...
            for i, group in enumerate(optimizer.param_groups):
                if 'initial_lr' not in group:
                    raise KeyError("param 'initial_lr' is not specified "
                                   ...
                                   " optimizer".format(i))
        self.base_lrs = list(map(lambda item: item['initial_lr'],
        ...
        self.t_mult = t_mult
        self.verbose = verbose
        self.base_weight_decays = list(map(lambda item: item['weight_decay'],
                                           optimizer.param_groups))

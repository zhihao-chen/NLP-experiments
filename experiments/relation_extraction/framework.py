...
"""
======================================
    Project Name: news_relation_extraction
    File Name: framework
    Author: czh
...
from data_loader_casrel import *
from transformers import BertConfig
from nlp.tools.utils import torch_init_model
...
class Framework(object):
    def __init__(self, con):
        ...
        self.bert_config = BertConfig(os.path.join("/home/chenzhihao/nezha-cn-base", "bert_config.json"))
    ...
    def train(self, model_pattern):
        ...
        ori_model = model_pattern(self.config, self.bert_config)
        torch_init_model(ori_model, os.path.join("/home/chenzhihao/nezha-cn-base", "pytorch_model.bin"))
        ...
        while data is not None:
            with torch.no_grad():
                ...
                if output:
                    ...
                    result = json.dumps(adict, ensure_ascii=False)
                    # fw.write(result + '\n')
    ...
    def testall(self, model_pattern):
        model = model_pattern(self.config, self.bert_config)
        path = os.path.join(self.config.checkpoint_dir, self.config.model_save_name)

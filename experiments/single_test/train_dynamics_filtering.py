...
from nlp.utils.selection_utils import read_training_dynamics
...
def read_data(input_file, seg_tag='\t'):
    all_datas = {}
    with codecs.open(input_file, encoding="utf8") as fr:
        ...
        for line in fr:
            ...
            if len(lst_line) != 3:
                continue
            if i in all_datas:
                logger.info(f"Found clash in IDs ... skipping example {i}.")
                continue
...
def variability_func_1(conf):
    return np.std(conf)


def variability_func_2(conf):
    return np.sqrt(np.var(conf) + np.var(conf) * np.var(conf) / (len(conf) - 1))


def closeness_func(conf):
    return conf * (1 - conf)


def compute_train_dy_metrics(training_dynamics_, include_ci=False, burn_out=100):
    """
    ...
    variability_func = variability_func_1
    if include_ci:  # Based on prior work on active bias (https://arxiv.org/abs/1704.07433)
        variability_func = variability_func_2
    threshold_closeness_func = closeness_func

    loss = torch.nn.CrossEntropyLoss()

    num_tot_epochs = len(list(training_dynamics_.values())[0]["logits"])
    ...
    for guid in tqdm.tqdm(training_dynamics_):
        correctness_trend = []
        true_probs_trend = []

        record = training_dynamics_[guid]
    ...
    df_train = pd.DataFrame([[i,
                              loss(torch.Tensor(logits[i]), torch.LongTensor(targets[i])).item() / len(
                                  training_dynamics_),
                              training_accuracy[i] / len(training_dynamics_)
...
def write_filtered_data(args, train_dy_metrics):
    ...
    original_train_file = os.path.join(os.path.join(args.data_dir, args.task_name), f"train.dataset")
    train_numeric = read_data(original_train_file)
    ...
    for fraction in [0.01, 0.05, 0.10, 0.1667, 0.25, 0.3319, 0.50, 0.75]:
        ...
        with open(os.path.join(outdir, f"train.txt"), "w") as outfile:
...
def plot_data_map(dataframe: pd.DataFrame,
    ...
    if not show_hist:
        ...
        gs = fig.add_gridspec(3, 2, width_ratios=[5, 1])  # 构造一个三行两列的图，两列的宽度比例是 5:1
...
class Config:
    filter = False
    plot = True
    model_dir = "/root/work2/work2/chenzhihao/NLP/experiments/output_file_dir/semantic_match/dy_logs"
    data_dir = "/root/work2/work2/chenzhihao/datasets/chinese-semantics-match-dataset/"
    plots_dir = '../cartography/'
...
def main():
    ...
    train_dy_metrics, _ = compute_train_dy_metrics(training_dynamics, args.include_ci, args.burn_out)

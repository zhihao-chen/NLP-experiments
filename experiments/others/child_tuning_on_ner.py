...
MODEL_CLASSES = (BertConfig, BertTokenizer, BertCrfForNer)


def init_model(num_labels, model_name_or_path=None):
    if model_name_or_path is None:
        model_name_or_path = args.model_name_or_path
    config_class, tokenizer_class, model_class = MODEL_CLASSES
...
def train(train_dataset, model, tokenizer):
    ...
    if args.mode == 'ChildTuning-D':
        gradient_mask = calculate_fisher(model, train_dataloader, args.reserve_p)
...
def predict(model, tokenizer, prefix=""):
    ...
    for step, batch in tqdm(enumerate(test_dataloader), desc="Predicting"):
        ...
        with torch.no_grad():
            inputs = {"input_ids": batch[0], "attention_mask": batch[1],
                      "token_type_ids": batch[2], "labels": None}
            outputs = model(**inputs)
